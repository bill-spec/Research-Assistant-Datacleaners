...
```{r}
joinFunctiion <- function(dfFound, dfTotal){
  
  df <- merge(dfFound, dfTotal, by = 0) %>% select(-Row.names)
  rownames(df) <- c(1890,1900,1910,1920,1930,1940,1950,1960,1970,1980,1990)
  return(df)
...
```{r}
excelData(newYorkExport, newYorkExport$Year, newYorkExport$Coarse.Geo)
```

Boston 
 #use at least 5 obs 
```{r}
excelData(boston2, boston2$Year, boston2$Area2)
```

DC 

```{r}
excelData(excelDC, excelDC$Year, excelDC$Area2)
```
