...
```{r}
intersectionCSV <- read.csv("C:/Users/Billy/Desktop/Sewer Project Geocoding/1900/chicago_intersections1900.csv")
needsMatchingCSV <- read.csv("C:/Users/Billy/Desktop/Sewer Project Geocoding/1900/1900geosnongoogle.csv", header = FALSE)
```

```{r}
names(intersectionCSV)[1] <- "FID" 
names(intersectionCSV)[4] <- "names" 
intersection <- intersectionCSV %>% select(FID, names)
intersection
```

```{r}
search <- needsMatchingCSV[,c(3,1,2)]
search
```
...
```{r}
key <- cleanIntersection(intersection)
LookUp(search, key, "V1");LookUp(search, key, "V2")
```
...
```{r}
df1 <- data.frame("ID" = 1:4, "x1" = c("forbes&10th","board&124th", "&5th", "fake&"), "x2" = c("10th&forbes","124th&board", "5th&", "&fake"))
...
```{r}
removeMissingRows <- function(dataframe){
  noMissingRows <- filter(dataframe, (dataframe[,2] != "" & dataframe[,3] != ""))
  return(noMissingRows)
}
```
...
cat("Key dataframe had", dim(dataframe_key)[1] ,"rows\n")
cat("Search dataframe had", dim(dataframe_needs_matched)[1] ,"rows\n")
...
#Text to blank
textToBlank <- c("^e$","^w$","^n$","^s$","^st$","^street$")
